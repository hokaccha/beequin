...
let KeyCode, Range, Position, Selection, SelectionDirection, monacoEditor;
class CMAdapter {
  static Pos = Pos;
  ...
  constructor(editor) {
    KeyCode = window.monaco.KeyCode;
    Range = window.monaco.Range;
    Position = window.monaco.Position;
    Selection = window.monaco.Selection;
    SelectionDirection = window.monaco.SelectionDirection;
    monacoEditor = window.monaco.editor;
    this.editor = editor;
    this.state = {
      keyMap: "vim",
    };
      ...
      find(back) {
        ...
        if (back) {
          ...
          const pos = lastSearch
            ? model.getPositionAt(model.getOffsetAt(lastSearch.getStartPosition()) + 1)
            : monacoPos;
